...
async def mainpageSHOW(token=Cookie(), db:AsyncSession=Depends(get_db), data=Body()):
    ...
    ads=(await db.execute(select(Ad).options(selectinload(Ad.reviews)).filter(Ad.subject.ilike(f'%{data['subject']}%'), Ad.price<=int(data['price'] if data['price'] else 999999999999), Ad.location.ilike(f'%{data['location']}%')))).all()
    for i in ads:
        rating=0
        counter=0
